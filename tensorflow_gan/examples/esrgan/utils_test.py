...
class UtilsTest(tf.test.TestCase):
  ...
  def test_visualize_results(self):
    ...
    self.assertIsNone(
        utils.visualize_results(self.lr_data, self.gen_data, self.hr_data))
  ...
  def test_fid_score(self):
    fid_value = utils.get_frechet_inception_distance(
        self.hr_data, self.gen_data, batch_size=2, num_inception_images=2)
    self.assertEqual(fid_value.dtype, tf.float32)

  def test_inception_score(self):
    is_score = utils.get_inception_scores(
        self.gen_data, batch_size=2, num_inception_images=2)
