...
HParams = collections.namedtuple('HParams', [
    'batch_size', 'hr_dimension', 'scale', 'model_dir', 'data_dir', 'num_steps',
    'num_inception_images', 'image_dir', 'eval_real_images'
])
  ...
  for step, (lr, hr) in enumerate(data.take(hparams.num_steps), start=1):
    ...
    if step % hparams.num_steps // 10 == 0:
      utils.visualize_results(
          lr, gen, hr, image_dir=hparams.image_dir, step=step)

    # Compute Frechet Inception Distance.
    fid_score = utils.get_frechet_inception_distance(
        hr, gen, hparams.batch_size, hparams.num_inception_images)
